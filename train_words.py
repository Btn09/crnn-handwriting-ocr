...
class IAMWordsDataset(Dataset):
    def __init__(self, words_list, img_dir, char_to_num):
    ...
    def __getitem__(self, idx):
        img_id, label = self.words_list[idx]
        p = img_id.split('-')
...
if __name__ == "__main__":
    ...
    words_data = parse_words_txt(words_txt_path)
    if not words_data: exit()

    dataset = IAMWordsDataset(words_data, words_img_dir, char_to_num)
    loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, 
                        num_workers=4, collate_fn=collate_fn)

    model = CRNN(len(alphabet)).to(device)

    weights_path = "words_model_final.pth"
    if os.path.exists(weights_path):
        model.load_state_dict(torch.load(weights_path, map_location=device))
        print(f"Previous training loaded: {weights_path}")
        print("Continuing training with pre-trained weights")

    criterion = nn.CTCLoss(blank=0, zero_infinity=True)
    optimizer = optim.Adam(model.parameters(), lr=0.0001)
    scheduler = StepLR(optimizer, step_size=3, gamma=0.5)

    print("Stage 1.5: Pro Words Training Started")
    best_loss = float('inf')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), "words_model_best.pth")
            print(f"New Record: Best model saved to words_model_best.pth (Loss: {best_loss:.4f})")
            
        torch.save(model.state_dict(), "words_model_final.pth")